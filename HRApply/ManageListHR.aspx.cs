...
{
    public partial class _ManageListHR : PageBaseManage
    {
        DataTable dtTable;
        DataTable dt;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Request.QueryString["AcctRole"] != null && Session["LoginEmail"] != null)
                {
                    ...
                    if (Session["AcctRoleName"] != null) 
                        AcctRoleName.Text = Session["AcctRoleName"].ToString();
                    if (AcctRoleName.Text.Equals("")) 
                        AcctRoleName.Text = crudObject.GetTitle(AcctAuditorSnEmpId);
                    //管理者
					BtnOutput.Visible = false; //非管理者才需要
                        LbOutput.Visible = false;
                        SelectTableHR.Visible = true; //20170509管理者權限才有的篩選器，現承辦人皆可使用
                        dtTable = crudObject.GetAllInApplyData(SelYear.SelectedValue, SelSemester.SelectedValue); //看到所有申請中的資料
                        Panel panelManage = (Panel)Master.FindControl("PanelManage");
                        panelManage.Visible = true;
                        SelectMethod.SelectedValue = "1";
                        LbSelectMethod.Text = "申請中資料";
                        BtnOutput.Visible = true; //非管理者才需要
                        LbOutput.Visible = true;
                        if (CBNowAudit.Checked)
                        {
                            dtTable = crudObject.GetApplyDataByAuditorNow(AcctAuditorSnEmpId, SelYear.SelectedValue, SelSemester.SelectedValue);   
                            //看到自己可以審核的資料-- 要抓輪到自己審核的
                        }
                        else
                        {
                            dtTable = crudObject.GetApplyDataByAuditor(AcctAuditorSnEmpId, SelYear.SelectedValue, SelSemester.SelectedValue); 
                        ...
                        if (dt.Rows.Count > 0 && AcctAuditorSnEmpId != "101071" && AcctAuditorSnEmpId != "104216"
                             && AcctAuditorSnEmpId != "108125" && AcctAuditorSnEmpId != "107122")
                        {
                    ...
                    {
                        ApplyDataMessage.Text = "目前沒有申請中的文件!";
                    }
                    else
                    {
        ...
        protected void BtnExport_OnClick(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn1 = new SqlConnection(connstring.ConnectionString))
            {
                ...
                if (Session["strWhere"] == null)
                    Session["strWhere"] = "";
                SqlDataAdapter dr = new SqlDataAdapter(@"SELECT distinct '' as '序號', a.AppYear as '學年度',
a.AppSemester as '學期', a.EmpSn as '申請人編號', 
a.AppSn as '申請單號','' as '審查性質', b.EmpSex as '性別',c.DeptName as '學院',
ISNULL(c.CollegeName,c.DeptName) as '系所',
c.unt_name_full as '聘任單位',b.EmpNameCN as '姓名',
f.JobAttrName as '專兼任別', g.JobTitleName as '應聘等級',
i.AuditProgressName as '申請狀態', e.KindName as '新聘升等',
f.JobAttrName as '應徵職別', b.EmpIdno as '身份證字號',
b.EmpBirthDay as '生日', b.EmpEmail as 'e-mail', b.EmpTelPri as '連絡電話1',
b.EmpTelPub as '連絡電話2', b.EmpCell as '連絡電話3',a.AppThesisAccuScore as '論文積分',
case when j.RPIDiscountTotal is not null then j.RPIDiscountTotal  else '0' end as '折抵積分',
a.AppRPIScore as 'RPI',b.EmpExpertResearch as '學術專長及研究',
ISNULL((select d.[ThesisName] from [ThesisScore] d where  a.EmpSn=d.EmpSn and d.IsRepresentative='1' FOR XML PATH('')),
ISNULL((SELECT[AppDDegreeThesisName] 
from[AppendDegree] zz  
where zz.AppSn = a.AppSn),  ISNULL((SELECT[AppDDegreeThesisNameEng] from[AppendDegree] z  where z.AppSn = a.AppSn), '')))   AS '代表著作/學位論文',
(select EduSchool + ' ' + EduDepartment + ' ' + CASE when S4.DegreeName IS NULL THEN '' when S4.DegreeName = '' THEN '' ELSE S4.DegreeName END  + ' ' + EduStartYM + '~' + EduEndYM +';  ' from[TeacherEdu] as S1  LEFT JOIN CDegree AS S4 ON S1.EduDegree = S4.DegreeNo WHERE S1.EmpSn = a.EmpSn FOR XML PATH('')) AS '學歷',( select ExpOrginization + ' ' + CASE when ExpJobTitle IS NULL THEN '' when ExpJobTitle = '' THEN '' ELSE ExpJobTitle END  + ' ' + ExpStartYM + '~' + ExpEndYM + ';  ' from TeacherExp as S2  
WHERE S2.EmpSn = a.EmpSn FOR XML PATH('')) AS '經歷'  ,
a.AppBuildDate as '申請案建立日期',a.AppModifyDate as '申請案送出日期' , a.AppKindNo, a.AppAttributeNo
FROM ApplyAudit a JOIN [EmployeeBase] b ON a.EmpSn = b.EmpSn  
INNER JOIN (select unt.unt_id,unt.unt_name_full,dpt.unt_name_full as 'DeptName',unt2.unt_name_full  as CollegeName  
from [PAPOVA].[TmuHr].[dbo].[s90_unit] unt LEFT JOIN[PAPOVA].[TmuHr].[dbo].[s90_unit] dpt on unt.dpt_id = dpt.unt_id 
LEFT JOIN[PAPOVA].[TmuHr].[dbo].[s90_unit] unt2 on unt.unt_id2 = unt2.unt_id) c ON a.AppUnitNo = c.unt_id COLLATE Chinese_Taiwan_Stroke_CI_AS 
JOIN [dbo].[CAuditKind] e ON a.AppKindNo = e.KindNo  JOIN[dbo].[CJobAttribute] f ON a.AppJobTypeNo = f.JobAttrNo JOIN[dbo].[CJobTitle] g ON a.AppJobTitleNo = g.JobTitleNo 
INNER JOIN[dbo].[AuditProgressStatus] i ON a.AppStage = i.AuditProgressNo   LEFT JOIN[dbo].[AppendPromote] j on a.AppSn = j.AppSn " + Session["strWhere"].ToString()  + " order by c.unt_name_full", conn1);
                    dr.Fill(ds);
            }
            ...
            {
                dr["審查性質"] = dr["AppKindNo"].ToString().Equals("1") ? strAttribute1[Int32.Parse(dr["AppAttributeNo"].ToString())] : strAttribute2[Int32.Parse(dr["AppAttributeNo"].ToString())];
            }
            dt.Columns.Remove("AppKindNo");
            dt.Columns.Remove("AppAttributeNo");
        ...
        protected void SearchGo_OnClick(object sender, EventArgs e)
        {
            ...
            if (SelConjunct1.SelectedValue == "-" &&
               SelConjunct2.SelectedValue == "-" &&
               SelConjunct3.SelectedValue == "-")
            ...
            if (SelConjunct1.SelectedValue != "-")
            {
                ...
                {
                    StrAnd += " and  " + SelItem1.SelectedValue + " = '" + SelChoose1.SelectedValue + "' ";
                }
            ...
            if (SelConjunct2.SelectedValue.Equals("and"))
            {
                if (SelChoose2.Visible == true)
                {
                    StrAnd += " and " + SelItem2.SelectedValue + " = '" + SelChoose2.SelectedValue + "' ";
                }
            ...
            if (SelConjunct2.SelectedValue.Equals("or"))
            {

                if (SelChoose2.Visible == true)
                {
                    StrOr += " or " + SelItem2.SelectedValue + " = '" + SelChoose2.SelectedValue + "' ";
                }
                if (TxtBox2.Visible && !TxtBox2.Text.Equals(""))
                {
                    if (SelHidden2.Equals("="))
            ...
            if (SelConjunct3.SelectedValue.Equals("and"))
            {
                if (SelChoose3.Visible == true)
                {
                    StrAnd += " and " + SelItem3.SelectedValue + " = '" + SelChoose3.SelectedValue + "' ";
                }
            ...
            if (SelConjunct3.SelectedValue.Equals("or"))
            {
                if (SelChoose3.Visible == true)
                {
                    StrOr += " or " + SelItem3.SelectedValue + " = '" + SelChoose3.SelectedValue + "' ";
                }
                if (TxtBox3.Visible && !TxtBox3.Text.Equals(""))
                {
                    if (SelHidden3.Equals("="))
            ...
            var method = SelectMethod.SelectedValue;
            var kind = SelectKind.SelectedValue;
            string strWhere = "";
            switch (method)
            {
                //尚未派發學年學期
                case "all":
                    ...
                    if (!StrAnd.Equals(""))
                    {
                        strWhere = "Where  a.AppKindNo = '" + kind + "' and a.AppYear = '" + SelYear.SelectedValue + "' and a.AppSemester =  '" + SelSemester.SelectedValue + "' " + StrAnd;
                    }
                    else
                    {
                        strWhere = "Where  a.AppKindNo = '" + kind + "' and a.AppYear = '" + SelYear.SelectedValue + "' and a.AppSemester =  '" + SelSemester.SelectedValue + "' ";
                    }
                    if (!StrOr.Equals(""))
                    {
                        strWhere += StrOr;
                ...
                case "1":
                    ...
                    if (!StrAnd.Equals(""))
                    {
                        strWhere = " Where a.AppStatus = 'False' and a.AppStage = '0' and a.AppKindNo = '" + kind + "' and a.AppYear = '" + SelYear.SelectedValue + "' and a.AppSemester =  '" + SelSemester.SelectedValue + "' " + StrAnd;
                    }
                    else
                    {
                        strWhere = " Where a.AppStatus = 'False' and a.AppStage = '0' and a.AppKindNo = '" + kind + "' and a.AppYear = '" + SelYear.SelectedValue + "' and a.AppSemester =  '" + SelSemester.SelectedValue + "'";
                    }
                    if (!StrOr.Equals(""))
                    {
                        strWhere += StrOr;
                ...
                case "2":
                    ...
                    if (!StrAnd.Equals(""))
                    {
                        strWhere = " Where a.AppStatus = 'True' and a.AppKindNo = '" + kind + "' and a.AppYear = '" + SelYear.SelectedValue + "' and a.AppSemester =  '" + SelSemester.SelectedValue + "' " + StrAnd;
                    }
                    else
                    {
                        strWhere = " Where a.AppStatus = 'True' and a.AppKindNo = '" + kind + "' and a.AppYear = '" + SelYear.SelectedValue + "' and a.AppSemester =  '" + SelSemester.SelectedValue + "' ";
                    }
                    if (!StrOr.Equals(""))
                    {
                        strWhere += StrOr;
            ...
            else
            {
                ...
                if (CBNowAudit.Checked)
                {
                    ...
                    dtTable = crudObject.GetApplyDataByAuditorNow(AcctAuditorSnEmpId, SelYear.SelectedValue, SelSemester.SelectedValue);   //看到自己可以審核的資料-- 要抓輪到自己審核的
        ...
        public void approveUnt_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            DataTable dt;
            String untData = approveUnt.SelectedValue.Trim();
            ...
            if (untData.Equals("會經手資料") && AcctRole != null && AcctRole.Equals("M"))
            {
                ...
                if (untData.Equals("會經手資料") && AcctRole != null && AcctRole.Equals("M"))
                {
                    ...
                    {
                        LbSelectMethod.Text = "需您經手審核的資料";
                        dtTable = crudObject.GetApplyDataByAuditor(AcctAuditorSnEmpId, SelYear.SelectedValue, SelSemester.SelectedValue); //看到自己可以審核的資料-- 所有經手的[目前是後者,並顯示已審,待審]
                    }
                ...
                else
                {
                    if (AcctRole != null && !AcctRole.Equals("M")) //管理者
                    {
                        ...
                        if (!allUnt.Equals(""))
                        {
                            dtTable = crudObject.GetApplyDataByUnit(allUnt, SelYear.SelectedValue, SelSemester.SelectedValue);
                            dt = new DataTable();
                            dtHidden = new DataTable();
                            dtHidden.Columns.AddRange(new DataColumn[] {new DataColumn("Name", System.Type.GetType("System.String")),
                                                   new DataColumn("UntName", System.Type.GetType("System.String")),
