...
namespace ApplyPromote
{
    ...
    public partial class PromoteEmp : PageBaseApply, System.Web.UI.ICallbackEventHandler
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                foreach (var k in thesisCoopClassificationItems.Keys)
                    this.ddlClassification.Items.Add(new ListItem(thesisCoopClassificationItems[k], k));
            }
            ...
            if (!IsPostBack)
            {
                foreach (var k in thesisCoopRDItems.Keys)
                    this.ddlRD.Items.Add(new ListItem(thesisCoopRDItems[k], k));
            }
        }
        ...
        protected void LoadDataBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (vEmp != null)
                {
                    ...
                    if (Request["AppSn"] != null && !Request["AppSn"].ToString().Equals(""))
                    {
                        int appSn = Convert.ToInt32(DES.Decrypt(Request["AppSn"].ToString()));
                        GVThesisCoop.DataSource = thesisCoopService.GetDataTableByAppSn(appSn);
        ...
        private VThesisCoop GetVThesisCoop()
        {
            VThesisCoop vThesisCoop = new VThesisCoop();
            vThesisCoop.EmpSn = Int32.Parse(Session["EmpSn"].ToString());

            if (Request["AppSn"] != null)
                Session["AppSn"] = new DESCrypt().Decrypt(Request["AppSn"].ToString());

            vThesisCoop.AppSn = Int32.Parse(Session["AppSn"].ToString());
            vThesisCoop.ProjectContent = this.txtProjectContent.Text;
            vThesisCoop.Classification = Convert.ToInt32(this.ddlClassification.SelectedValue);
            vThesisCoop.RD = Convert.ToInt32(this.ddlRD.SelectedValue);
            vThesisCoop.Contribute = Convert.ToInt32(this.txtContribute.Text);
            vThesisCoop.Contribute = vThesisCoop.Contribute > 100 ? 100 : (vThesisCoop.Contribute < 0 ? 0 : vThesisCoop.Contribute);
            vThesisCoop.Total = Convert.ToInt32(vThesisCoop.Classification * vThesisCoop.RD * vThesisCoop.Contribute / 100);
            vThesisCoop.ID = String.IsNullOrEmpty(this.hfID.Value) ? 0 : Convert.ToInt32(this.hfID.Value);


            if (ThesisCoopUploadFU.HasFile)
            {
                if (ThesisCoopUploadFU.FileName != null && checkName(ThesisCoopUploadFU.FileName))
                {
                    ThesisCoopUploadCB.Checked = true;

                    fileLists.Add(ThesisCoopUploadFU);
                    ProcessUploadFiles(Int32.Parse(TbEmpSn.Text.ToString()));
                    vThesisCoop.UploadFileName = ThesisCoopUploadFU.FileName;


                    if (checkName(ThesisCoopUploadFU.FileName) && Session["FolderPath"] != null)
                        ThesisCoopUploadFU.PostedFile.SaveAs(Session["FolderPath"] + ThesisCoopUploadFU.FileName);
                    else
                ...
                vThesisCoop.UploadFileName = ThesisCoopUploadCB.Checked ? "" : this.hfUploadFileName.Value;

            return vThesisCoop;
        }

        private string ValiateThesisCoopForm()
        {
            string msg = "";
            if (Session["EmpSn"] == null)
            {
            ...
            else
            {
                ...
                if (String.IsNullOrEmpty(txtContribute.Text))
                    msg = "請填寫貢獻比例";
                else if (!decimal.TryParse(txtContribute.Text, out o))
                {
                    msg = "貢獻比例請填寫數字格式";
                }
                ...
                //        thesisCoopService.Create(vThesisCoop);
        ...
        private void BindGVThesisCoop()
        {
            this.txtProjectContent.Text = "";
            this.ddlClassification.SelectedValue = "10";
            this.ddlRD.SelectedValue = "1";
            this.txtContribute.Text = "0";
            GVThesisCoop.DataSource = thesisCoopService.GetDataTableByAppSn(Convert.ToInt32(Session["AppSn"]));
            GVThesisCoop.DataBind();
        }
        /// <summary>
        /// 新增産學合作加分項目
        ...
        {
            string msg = ValiateThesisCoopForm();
            if (!String.IsNullOrEmpty(msg))
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "message", "$(function() { $('#modalThesisCoop').modal('show'); }); alert('" + msg + "');", true);
                return;
            }
            VThesisCoop vThesisCoop = GetVThesisCoop();
            this.thesisCoopService.Create(vThesisCoop);
            BindGVThesisCoop();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void lnkThesisCoopCreate_Click(object sender, EventArgs e)
        {
            this.hfID.Value = "";
            this.hfUploadFileName.Value = "";
            this.txtProjectContent.Text = "";
            this.ddlClassification.SelectedValue = "10";
            this.ddlRD.SelectedValue = "1";
            this.txtContribute.Text = "0";
            this.ThesisCoopCreate.Visible = true;
            this.ThesisCoopUpdate.Visible = false;

            ThesisCoopUploadFU.Visible = true;
            ThesisCoopUploadFUName.Visible = false;
            ThesisCoopUploadCB.Checked = false;
            ThesisCoopHyperLink.Text = "";
            ThesisCoopHyperLink.Visible = false;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void ThesisCoopModData_Click(object sender, EventArgs e)
        {

            var deleteLink = (Control)sender;
            GridViewRow row = (GridViewRow)deleteLink.NamingContainer;
            int id = Convert.ToInt32(this.GVThesisCoop.DataKeys[row.RowIndex].Values[0].ToString());
            VThesisCoop vThesisCoop = this.thesisCoopService.Get(id);
            this.hfID.Value = vThesisCoop.ID.ToString();
            this.hfUploadFileName.Value = vThesisCoop.UploadFileName;
            this.txtProjectContent.Text = vThesisCoop.ProjectContent;
            this.ddlClassification.SelectedValue = vThesisCoop.Classification.ToString();
            this.ddlRD.SelectedValue = vThesisCoop.RD.ToString();
            this.txtContribute.Text = vThesisCoop.Contribute.ToString("D");

            this.ThesisCoopCreate.Visible = false;
            this.ThesisCoopUpdate.Visible = true;

            //上傳論文
            if (!String.IsNullOrEmpty(vThesisCoop.UploadFileName))
            {
                ThesisCoopUploadCB.Checked = true;
                if (!String.IsNullOrEmpty(vThesisCoop.UploadFileName) && checkName(vThesisCoop.UploadFileName))
                {
                    ThesisCoopHyperLink.NavigateUrl = getHyperLink(vThesisCoop.UploadFileName);
                    ThesisCoopHyperLink.Text = vThesisCoop.UploadFileName;
                }
                ThesisCoopHyperLink.Visible = true;
                ThesisCoopUploadFU.Visible = true;
                ThesisCoopUploadFUName.Visible = false;
                ThesisCoopUploadFUName.Text = vThesisCoop.UploadFileName;
            }
            else
            {
                ThesisCoopUploadFU.Visible = true;
        ...
        protected void ThesisCoopUpdate_Click(object sender, EventArgs e)
        {
            ...
            VThesisCoop vThesisCoop = GetVThesisCoop();
            this.thesisCoopService.Update(vThesisCoop);
            BindGVThesisCoop();
        }
        ...
        protected void ThesisCoopDeleteData_Click(object sender, EventArgs e)
        {
            if (Session["EmpSn"] == null)
            ...
            else
            {
                ...
                thesisCoopService.Delete(id);

                GVThesisCoop.DataSource = thesisCoopService.GetDataTableByAppSn(Convert.ToInt32(Convert.ToString(Session["AppSn"])));
                GVThesisCoop.DataBind();
            }
        }
            ...
            else
            {
                ...
                if (crudObject.UpdatePartial(vApplyAudit))
                {
                    ...
                    if (RPIDiscountNo.Checked)
                    {
                        ...
                        vAppendPromote.RPIDiscountScore5 = "0";
                    ...
                    else
                    {
                        ...
                        vAppendPromote.RPIDiscountScore5 = "0";
                        List<VThesisCoop> coops = this.thesisCoopService.GetByAppSn(Convert.ToInt32(Session["AppSn"]));
                        if (coops.Any())
                        {
                            vAppendPromote.RPIDiscountScore5 = coops.Sum(c => c.Total).ToString();
                        }


                        int discountTotal = Int32.Parse(vAppendPromote.RPIDiscountScore1) + Int32.Parse(vAppendPromote.RPIDiscountScore2) + Int32.Parse(vAppendPromote.RPIDiscountScore3) + Int32.Parse(vAppendPromote.RPIDiscountScore4) + Int32.Parse(vAppendPromote.RPIDiscountScore5);
                        //折抵論文積分最高180
                        if (discountTotal > 180)
                            discountTotal = 180;


                        vAppendPromote.RPIDiscountTotal = discountTotal.ToString();
                        RPIDiscountTotal.Text = discountTotal.ToString();
                        RPIDiscountTotal1.Text = discountTotal.ToString();
                        AppRPITotalScore.Text = (Convert.ToDecimal(String.IsNullOrEmpty(AppPThesisAccuScore1.Text) ? "0": AppPThesisAccuScore1.Text) + Convert.ToDecimal(discountTotal)).ToString();
                    }
